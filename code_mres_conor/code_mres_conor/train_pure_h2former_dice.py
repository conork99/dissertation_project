...
import os
import argparse
from lib.pure_h2former_bce_decoders import PraNet
...
parser.add_argument('--exp', type=str,
                    default='DR_grading_seg_pure_h2former_bce+dice', help='model_name')
...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # SE_loss = dice_loss(torch.argmax(lateral_map_1, dim =1), label_se.squeeze().float())
            # EX_loss = dice_loss(torch.argmax(lateral_map_3, dim=1), label_ex.squeeze().float())
            # MA_loss = dice_loss(torch.argmax(lateral_map_2, dim=1), label_ma.squeeze().float())
            # HE_loss = dice_loss(torch.argmax(lateral_map_4, dim=1), label_he.squeeze().float())
