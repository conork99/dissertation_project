...
class UNet(torch.nn.Module):
    ...
    def __init__(self, n_classes, batch_norm=True):
        ...
        self.segment1 = nn.Conv2d(self.filter_sizes[0], self.n_classes, kernel_size=1)
        self.segment2 = nn.Conv2d(self.filter_sizes[0], self.n_classes, kernel_size=1)
        self.segment3 = nn.Conv2d(self.filter_sizes[0], self.n_classes, kernel_size=1)
        self.segment4 = nn.Conv2d(self.filter_sizes[0], self.n_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        y_pred1 = self.segment1(x)
        y_pred2 = self.segment2(x)
        y_pred3 = self.segment3(x)
        y_pred4 = self.segment4(x)

        return y_pred1, y_pred2, y_pred3, y_pred4, grade_pred
    ...
    def contract(self):
        ...
        for i, size in enumerate(self.filter_sizes):
            ...
            conv1 = nn.Conv2d(old, size, kernel_size=3, padding=1)  # , padding=1
            conv2 = nn.Conv2d(size, size, kernel_size=3, padding=1)  # , padding=1
            relu = nn.ReLU(True)
    ...
    def expand(self):
        ...
        for i, size in enumerate(expand_filters):
            up = nn.ConvTranspose2d(old, size, kernel_size=3, stride=2, padding=1, output_padding=1)  # kernel_size=2, stride=2
...
if __name__ == "__main__":
    # A full forward pass
    im = torch.randn(1, 3, 128, 128).cuda()
