...
class DenseUNet(nn.Module):
    def __init__(self, n_classes, growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=64, bn_size=4,
        ...
        self.decoder = _DenseUNetDecoder(self.skip_connections, growth_rate, block_config, num_init_features, bn_size,
                                         drop_rate, downsample)
        self.classifier1 = nn.Conv2d(num_init_features, n_classes, kernel_size=1, stride=1, bias=True)
        self.classifier2 = nn.Conv2d(num_init_features, n_classes, kernel_size=1, stride=1, bias=True)
        self.classifier3 = nn.Conv2d(num_init_features, n_classes, kernel_size=1, stride=1, bias=True)
        self.classifier4 = nn.Conv2d(num_init_features, n_classes, kernel_size=1, stride=1, bias=True)
    ...
    def forward(self, x):
        ...
        return y1, y2, y3, y4, grade_pred

if __name__ == '__main__':
    ras = DenseUNet(n_classes=2).cuda()
