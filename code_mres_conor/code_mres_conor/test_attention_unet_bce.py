...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for i_batch, (sampled_batch, sampled_name) in enumerate(testloader):
            volume_batch, label_se, label_ex, label_ma, label_he = sampled_batch['img'], \
                sampled_batch['SE_mask'], \
                sampled_batch['EX_mask'], sampled_batch['MA_mask'], sampled_batch['HE_mask']
            # edge_batch_com = edge_batch[:, 0, :, :] + edge_batch[:, 1, :, :]
            volume_batch, label_se, label_ex, label_ma, label_he = volume_batch.float().cuda(), label_se.float().cuda(), \
                label_ex.float().cuda(), label_ma.float().cuda(), label_he.float().cuda()
