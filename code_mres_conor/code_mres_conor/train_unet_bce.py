...
parser.add_argument('--exp', type=str,
                    default='unet_segmentation_bce', help='model_name')
...
parser.add_argument('--decay_rate', type=float,
                        default=0.9, help='decay rate of learning rate')
...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            lateral_map_4, lateral_map_3, lateral_map_2, lateral_map_1 = model(volume_batch) # SE, EX, MA, HE, grading
            # ori_mask, grade_pred = model(volume_batch)

            # lateral_map_1 = F.interpolate(lateral_map_1, size=(128, 128), mode='bilinear', align_corners=True)
            # lateral_map_2 = F.interpolate(lateral_map_2, size=(128, 128), mode='bilinear', align_corners=True)
            # lateral_map_3 = F.interpolate(lateral_map_3, size=(128, 128), mode='bilinear', align_corners=True)
            # lateral_map_4 = F.interpolate(lateral_map_4, size=(128, 128), mode='bilinear', align_corners=True)
            ...
            SE_loss = ce_loss(lateral_map_1.squeeze(), label_se.squeeze().long()) + dice_loss(torch.argmax(lateral_map_1, dim =1).squeeze(), label_se.squeeze().float())
            EX_loss = ce_loss(lateral_map_3.squeeze(), label_ex.squeeze().long()) + dice_loss(torch.argmax(lateral_map_3, dim=1).squeeze(), label_ex.squeeze().float())
            MA_loss = ce_loss(lateral_map_2.squeeze(), label_ma.squeeze().long()) + dice_loss(torch.argmax(lateral_map_2, dim=1).squeeze(), label_ma.squeeze().float())
            HE_loss = ce_loss(lateral_map_4.squeeze(), label_he.squeeze().long()) + dice_loss(torch.argmax(lateral_map_4, dim=1).squeeze(), label_he.squeeze().float())
            ...
            logging.info(
                'iteration %d : loss : %f, se_loss: %f, ex_loss: %f, ma_loss: %f, he_loss: %f' %  #, , edge_disc_loss: %f
                (iter_num, loss.item(), SE_loss.item(), EX_loss.item(),  MA_loss.item(), HE_loss.item()))  # edge_cup_loss.item(), edge_disc_loss.item()
        ...
        if epoch_num % 10 == 0: # epoch_num > 0 and
            ...
            with torch.no_grad():
                ...
                for i_batch, (sampled_batch, sample_name) in enumerate(testloader):
                    ...
                    lateral_map_4, lateral_map_3, lateral_map_2, lateral_map_1 = model(
                    ...
                    single_metric_se = calculate_metric_percase(lateral_map_1, label_se.astype(np.uint8))
                    ...
                    single_metric_he = calculate_metric_percase(lateral_map_4, label_he.astype(np.uint8))
                    # print(single_metric_ex)
                ...
                if avg_metric_ex > best_seg:
                    best_seg = avg_metric_ex
                    save_mode_path = os.path.join(snapshot_path, 'best_model_seg.pth')
                    torch.save(model.state_dict(), save_mode_path)
                    logging.info("save model to {}".format(save_mode_path))
                    logging.info('best seg performance dice score:{}'.format(best_seg))

                logging.info('seg performance dice score:{}'.format(best_seg))
                # logging.info('iteration %d : dice : %f' % (iter_num, avg_metric))
                # writer.add_scalar('val/avg_dice', avg_metric, iter_num)
                writer.add_scalar('val/avg_dice_se', avg_metric_se, iter_num)
                writer.add_scalar('val/avg_dice_ex', avg_metric_ex, iter_num)
                writer.add_scalar('val/avg_dice_he', avg_metric_he, iter_num)
                writer.add_scalar('val/avg_dice_ma', avg_metric_ma, iter_num)
                ...
                # writer.add_scalar('val_acc/avg_f1', f1_score, iter_num)
                # writer.add_scalar('val_acc/avg_pre', precision_score, iter_num)
                # writer.add_scalar('val_acc/avg_sen', sen, iter_num)
                # writer.add_scalar('val_acc/avg_spe', spe, iter_num)
                # writer.add_scalar('val_acc/avg_auc', auc, iter_num)
